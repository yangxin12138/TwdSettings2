 ...
 */
    private WifiConfiguration getMatchingWifiConfig(ScanResult result) {
        NetworkInfo networkInfo;

            try {
                Method method = WifiManager.class.getDeclaredMethod("getMatchingWifiConfig", new Class[0]);
                WifiConfiguration config = (WifiConfiguration)method.invoke(mWifiManager, result, null);
                return config;
                //networkInfo = mConnectivityManager.getNetworkInfo((Network) (method).invoke(mWifiManager, new Object[0]));
        ...
        if (configs != null) {
            ...
            for (WifiConfiguration config : configs) {
                try {
                    ...
                    if (config_selfAdded && config_numAssociation == 0) {
                        continue;
                    }
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            for (ScanResult result : results) {
                ...
                if (!found && mIncludeScans) {
                    ...
                    if (result.isPasspointNetwork()) {
                        WifiConfiguration config = getMatchingWifiConfig(result);//mWifiManager.getMatchingWifiConfig(result);
                        if (config != null) {
    ...
    private final class WifiTrackerNetworkCallback extends ConnectivityManager.NetworkCallback {
        public void onCapabilitiesChanged(Network network, NetworkCapabilities nc) {
            ...
            try {
            if (network.equals(WifiManager.class.getDeclaredMethod("getCurrentNetwork", new Class[0]).invoke(mWifiManager, new Object[0]))) {
