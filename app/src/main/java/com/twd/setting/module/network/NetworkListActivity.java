 ...
 * @Author:Yangxin
 * @Description:
 * @time: Create in 11:45 2024/3/14
 */
public class NetworkListActivity extends AppCompatActivity
    ...
    private void updateWifi(){
        if (mConnectivityListener.isWifiEnabledOrEnabling()){
            Log.d(TAG, "updateWifi: isWifiEnabledOrEnabling is true");
            updateWifiList();
            return;
        }
    }
    ...
    private void updateWifiList(){
        ...
        /*if (mNoWifiUpdateBeforeMillis > l){
            ...
            mHandler.postDelayed(mInitialUpdateWifiListRunnable,mNoWifiUpdateBeforeMillis -1);
            return;
        }*/
        ...
        if ((wifiAccessPoints != null) && (wifiAccessPoints.size() != 0)){
            Log.d(TAG, "updateWifiList: 更新列表");
            adapter.notifyWifiAccessPoints();
        }
    }

    @Override
    public void onAccessPointChanged(WifiAccessPoint paramWifiAccessPoint) {
        WifiAccessPoint accessPoint = (WifiAccessPoint) paramWifiAccessPoint.getTag();
        if (accessPoint != null){
